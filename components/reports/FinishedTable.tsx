...
export default function FinishedTable({ items }: { items: Item[] }) {
  if (items.length === 0) {
    return <p className="text-center py-8" style={{ color: PALETTE.deepTeal300 }}>Belum ada item selesai & lunas</p>;
  }
    ...
    <table className="w-full">
      <thead>
        <tr className="border-b" style={{ borderColor: PALETTE.deepTeal700 }}>
          ...
          <th className="text-left py-3 px-4 text-sm font-medium" style={{ color: PALETTE.deepTeal300 }}>Qty</th>
          <th className="text-left py-3 px-4 text-sm font-medium" style={{ color: PALETTE.deepTeal300 }}>Terkirim</th>
        </tr>
      </thead>
      <tbody>
        {items.map((item) => (
          <tr key={item.id} className="border-b hover:bg-gray-50" style={{ borderColor: PALETTE.deepTeal900 }}>
            ...
            <td className="py-3 px-4" style={{ color: PALETTE.deepTeal300 }}>
              {item.quantity} {item.unit}
            </td>
            <td className="py-3 px-4" style={{ color: PALETTE.deepTeal300 }}>{formatDate(item.deliveredAt)}</td>
          </tr>
        ))}
      </tbody>
    </table>
